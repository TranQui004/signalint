...
- Retention: cap file-cache rows at 10,000. Refresh `timestamp` on a cache hit and evict the least-recently-used rows by that column after writes; `engine_state` remains one row per engine.
...
- **Whole-program engines (tsc):** never pass source-file flags. Inspect the resolved root config with `tsc --showConfig`. For an ordinary config without a `references` field, invoke with `--project` and rely on TypeScript's own `--incremental` mode with a persisted `.tsbuildinfo` file (stored at `.signalint/cache/tsc.tsbuildinfo`). For a solution-style root config containing `references`, invoke `tsc --build <root-config> --incremental` instead: plain `--project` mode sees a typical solution root's `files: []` but does not traverse its referenced sub-projects, creating a silent false negative. Build mode manages build information per referenced project and rejects the project-mode `--tsBuildInfoFile` and `--skipLibCheck` command-line overrides, so those flags are intentionally omitted in this branch. Signalint's own file-hash cache is used only to decide **whether to invoke tsc at all** for a given `check_files` call (skip the subprocess entirely if no file relevant to the TS program graph changed since the last check) — not to decide which files tsc analyzes internally. Once invoked, tsc always sees the whole project graph, as it must to be correct.

**Amendment log (Phase 6 dogfooding, 2026-07):** solution-style root configs changed from `--project` to `--build` after direct comparison showed that `--project` returned zero issues for `files: []` while `--build` traversed the references and found real TS1484 and TS2769 diagnostics. A regression fixture plus compiled-dist verification confirms that a broken referenced sub-project produces TS2322.
...
| Old loop signatures remain actionable indefinitely across checks and server restarts | Post-Phase-6 backlog: surface warnings only while a signature is currently active/reappearing; bound retained appearances by a time or check-count window; compact or rotate `session.jsonl`; add an explicit session-reset command instead of treating manual file deletion as a live-process reset |
| Windows `npm link` shim makes `signalint-mcp` exit before the MCP handshake, producing initialize/EOF | Fixed by canonicalizing `import.meta.url` and `process.argv[1]` through real paths before main-module detection; covered by a junction/symlink regression test and a documented direct-Node fallback |
| Windows `npm link` shim makes `signalint stats` exit 0 with no output | Same linked-path root cause and realpath fix as the MCP entrypoint; verified through the actual generated `.cmd` shim, with the compiled CLI path documented as a fallback |
...
| Vitest has no coverage measurement or enforced thresholds (external security audit, 2026-07-31; reconfirmed by final CL1 review) | Backlog: establish a measured baseline, add meaningful per-layer thresholds, and raise them without incentivizing low-value tests |
...
| `SessionMemory` performs blocking `readFileSync` and unbounded full-log replay during startup (external security audit, 2026-07-31) | Resolved pre-launch: startup reads at most the latest 5,000 entries from a bounded tail; writes rotate at 10 MiB and retain one bounded backup |
...
| CI tests only Node 22.x although `package.json` promises Node 20.19+ support (external security audit, 2026-07-31) | Resolved pre-launch: retain the Node 22 cross-platform matrix and add an Ubuntu Node 20.19 compatibility job; because canonical pnpm 11 itself requires Node 22.13+, that job installs the locked tree under Node 22, switches to Node 20.19, rebuilds the native SQLite addon for that ABI, and runs tools directly |
| Session-stat maximum latency spreads an unbounded array into `Math.max` (final pre-launch reviews, 2026-08-01) | Resolved pre-launch: reduce iteratively with a 150,000-entry regression test |
...
| Biome hardcodes `fixable: false` while Oxlint derives structured fixability, under-claiming fixes and skewing cluster priority (final CL1 review, 2026-08-01) | Backlog: map Biome reporter fix data into `fixable` honestly |
...
| Phase 2 benchmark mutates repository fixtures and uses a hard `<300ms` wall-clock assertion that can flake (final release-readiness review, 2026-08-01) | Backlog: copy fixtures to a temporary directory, retain call counts as correctness proof, and make elapsed time informational |
| Package release has no `prepublishOnly` lint/test gate, packed-tarball smoke gate, or provenance configuration (final release-readiness review, 2026-08-01) | Backlog before first real npm publish: add all three release controls |
| `better-sqlite3` is native and ABI-bound, while replacing it with `node:sqlite` would drop the newly tested Node 20 line (final release-readiness review, 2026-08-01) | Keep `better-sqlite3` for v1; revisit after Node 20 support is intentionally dropped |
| Disabled-by-default `@biomejs/biome` remains a hard dependency and adds install cost for every user (final release-readiness review, 2026-08-01) | Backlog: consider an optional peer dependency; do not change dependency shape without explicit approval |
| MCP tools omit `outputSchema` and `structuredContent`, returning pretty-printed JSON text instead of declared structured diagnostics (final CL1 review, 2026-08-01) | Highest-value v1.1 MCP backlog item: add structured output and drop pretty-printing on the MCP path together |
| Tier 3 maintenance debt: duplicated `compareIssues`/`isRecord`/`normalizeFile`/`readString`; duplicate path/default-exclusion passes; Oxlint aborts a batch on one malformed location; up to 512 file reads fan out without a concurrency bound; Signalint-version resolution has no fallback; shutdown has no timeout; POSIX termination skips SIGTERM grace (final CL1 review, 2026-08-01) | Normal maintenance backlog; no pre-launch urgency |
| No Dependabot or `pnpm audit` CI gate, and GitHub Actions use mutable version tags rather than commit SHAs (external security audit, 2026-07-31) | Weekly GitHub Actions Dependabot is configured; pnpm package updates remain blocked because [GitHub supports pnpm only through v10](https://docs.github.com/en/code-security/reference/supply-chain-security/supported-ecosystems-and-repositories) while Signalint keeps pnpm 11.9.0 canonical; backlog remains to add an audit policy, enable pnpm updates when supported, and pin Actions to reviewed commit SHAs |
